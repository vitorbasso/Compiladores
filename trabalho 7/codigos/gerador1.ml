...
and gen_return ts exp current = 
  let entrada = Hashtbl.find ts !current_func in
    (match entrada with
    | EntFun entFun ->
      let retorno = entFun.tiporetorno in
      (match retorno with
        | Some _ -> (match exp.valor with
          | ( Some ExpVar _) -> let reg = endereco ts ( converte_tipo_expr_option exp.valor) current in [sprintf "return v%d" (converte_int reg)]
          | (Some ExpInt i ) -> let reg = prox_registrador() in [opBinRn "const/16" reg i] @ [sprintf "return v%d" reg]
          | (Some ExpString s) -> let reg = prox_registrador() in [ opBinRString "const-string" reg s] @ [sprintf "return v%d" reg]
          | _ -> [sprintf "return aceitando apenas variaveis, int e string"])
        | _ -> failwith "gen_return"
      )
    | _ -> failwith "gen_return - variavel encontrada")

and gen_chamadaFuncao ts nomeFun arg current =
  let entrada = Hashtbl.find ts nomeFun in
    (match entrada with
    | EntFun entFun ->
      let params = entFun.param in
      let retorno = entFun.tiporetorno in
      let cod1 = "invoke-static {" in
      let (cod2, cod0) = regist_arg arg ts params current in
      let cod3 = "}, L" ^ !nomePrograma ^ ";->" ^ nomeFun ^ " (" in
      let cod4 = (tipos_params params) ^ ")" in
      (match retorno with
        | Some a -> let cod5 = tipoRetorno a in
        let codFinal = cod0 ^ cod1 ^ cod2 ^ cod3 ^ cod4 ^ cod5 in [codFinal]
        | _ -> failwith "funcao com retorno none: gen_chamadaFuncao")
    | _ -> failwith "gen_chamadaFuncao - variavel encontrada")

and gen_chamadaFuncaoAtrib ts exp nomeFun arg current =
    let lista = gen_chamadaFuncao ts nomeFun arg current in
    let entrada = Hashtbl.find ts nomeFun in
    (match entrada with
      | EntFun entFun ->
        let regExp = endereco ts (converte_tipo_expr_option exp.valor) current in
        let reg = prox_registrador() in 
        lista @ [sprintf "move-result v%d" reg] @ [spintf "move v%d, v%d" (converte_int regExp) reg]
      | _ -> failwith "gen_chamadaFuncaoAtrib - variavel encontrada")

and gen_cmd ts c current =
    match c with
    | CmdSe (teste, cmdsE, cmdsS) -> gen_if ts teste cmdsE cmdsS current
    | CmdAtrib (esq, dir) -> gen_atrib ts esq dir current
    | While (teste, cmds) -> gen_while ts teste cmds current
    | CmdFuncao (id, param, cmds) -> gen_func ts id param cmds
    | CmdOut (exp) -> gen_print ts exp current
    | CmdIn (exp1, exp2) -> gen_input ts exp1 exp2 current
    | CmdRetorno (exp) -> gen_return ts exp current
    | ChamadaFuncaoAtrib (exp, nomeFun, argum) -> gen_chamadaFuncaoAtrib ts exp nomeFun argum current
    | _ -> failwith "gen_cmd: comando nao implementado"

and gen_cmds ts cs current = 
  match cs with
  | c :: cs -> let lista = gen_cmd ts c.vcmd current in lista @ gen_cmds ts cs current
  | [] -> []

and tipos_params ps =
  match ps with
  | (p1, p2) :: ps -> let str = tipoRetorno (converte_tipo_base_option p2.tipagem)
  in str ^ tipos_params ps
  | [] -> ""
