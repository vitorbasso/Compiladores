...
let rec verifica_cmd amb tiporet cmd =
              ...
              begin
                  (match (Amb.busca amb id) with
                    Amb.EntVar tipo ->
                      ...
                      "ScanInt com tipos diferentes: %s = %s"
                      tipo (snd expt) in
                      CmdScanInt (fst expt)
                    | Amb.EntFun _ ->
                  let msg = "nome de funcao usado como nome de variavel: " ^ id in
          ...
          begin
              (match (Amb.busca amb id) with
                Amb.EntVar tipo ->
                  let expt = infere_exp amb exp in
                  let _ = mesmo_tipo pos
                  "ScanFloat com tipos diferentes: %s = %s"
                  tipo (snd expt) in
                  CmdScanFloat (fst exp)
                | Amd.EntFun _ ->
            let msg = "nome de funcao usado como nome de variavel: " ^ id in
            failwith (msg_erro_pos pos msg))
            end
            ...
            begin
              (match (Amb.busca amb id) with
                Amb.EntVar tipo ->
                  let expt = infere_exp amb exp in
                  let _ = mesmo_tipo pos
                  "ScanString com tipos diferentes: %s = %s"
                  tipo (snd expt) in
  ...
  | CmdWhile (exp_cond, comandos) ->
        let (expCond, expT) = infere_exp amb exp_cond in
        let comandos_tipados =
          (match expT with
            | A.TipoBool -> List.map (verifica_cmd amb tiporet) comandos
            | _ -> let msg = "Condicao deve ser tipo Bool" in
                  failwith (msg_erro_pos (posicao exp_cond) msg))
